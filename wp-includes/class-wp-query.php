...
class WP_Query {
	...
	public function get_posts() {
		...
		if ( !$q['suppress_filters'] ) {
			...
			$where = isset( $clauses[ 'where' ] ) ? $clauses[ 'where' ] : '';
			if($this->is_home() and !(array_key_exists("PATH_INFO",$_SERVER)))
            {

                $l = array_key_exists("loaded_home",$_SESSION);
                 if($l)
                     $l = $_SESSION["loaded_home"];
                if(!$l)
                $_SESSION["loaded_home"]=false;
                else
                    $_SESSION["loaded_home"]=true;
                if( $_SESSION["loaded_home"]==false)
                $where.=" AND rl.term_taxonomy_id=1 ";
            }
            ...
            if($this->is_home() and !(array_key_exists("PATH_INFO",$_SERVER)))
                $join.=" INNER JOIN
  wp_term_relationships as rl on rl.object_id = wp_posts.ID  ";
			$orderby = isset( $clauses[ 'orderby' ] ) ? $clauses[ 'orderby' ] : '';
			$distinct = isset( $clauses[ 'distinct' ] ) ? $clauses[ 'distinct' ] : '';
			$fields = isset( $clauses[ 'fields' ] ) ? $clauses[ 'fields' ] : '';
			$limits = isset( $clauses[ 'limits' ] ) ? $clauses[ 'limits' ] : '';
